// Подпрограмма проверки необходимости выполнения аппаратного сброса
void doHardReset() {
...
void changeState() {
  ...
  if (currentState == Pass && isLivePass) { // Если пролив завершён (newState ведь не равен currentState), и он был боевым
    ...
    deugValueChangesMillis = millis() - debugValueChangeBegin;
  }
  else {
...
void makeSendString(String& s) { // Формирование строки для объекта AsyncEventSource
  ...
  if (waterStreamValue > 0) {
    s += "\/";
    s += passTimeInMillis / waterStreamValue; // Время пролива, делённое на объём
  }
  s += "¿";
...
void lightIndication() { // Используем аппаратный фэйдинг
  if (currentState != Diagnostics) { // В режиме диагностики выключаем автоматическое управление светом
    if (isEspressoHeatingOn) { // Если нагрев включен,
      if (isTemperatureReached) { // и температура в приемлемом диапазоне, светим на постоянку:
        ledcWrite(TANK_LED, LEDC_TARGET_DUTY);
      }
...
// Подстановка значений. Используется на страничке настроек
String processor(const String& var) { // Подстановка значений в плейсхолдеры на страницах, этого требующих
  if (var == "P1") return P1;
  if (var == "P2") return P2;
  if (var == "P3") return P3;
  if (var == "P4") return P4;
  ...
  if (var == "P7") return P7;
  if (var == "P8") return "12345678"; // ну не отправлять же на страницу пароль...
  if (var == "WiFiNetworks") {
...
// Вычисление температуры NTC-термистора
...
void initParams() { // Актуализируем глобальные переменные, хранящие значения параметров:
  ...
  preferences.end(); // Закрываем настройки
  // Теперь в переменных P1...Pn либо значения параметров, либо значения зашитые в прошивке
}
