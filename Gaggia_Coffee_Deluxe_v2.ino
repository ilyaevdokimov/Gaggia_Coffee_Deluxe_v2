...
#include <MAX31865_NonBlocking.h> // Неблокирующая библиотека PT100-термистора budulinek
#include "5_Enums.h" // Собственные типы данных. Вынесены в заголовочный файл, чтобы их можно было использовать в качестве параметров и возвращаемых значений функций
...
//volatile uint64_t debugValueChangedTime; // Время изменения значения
...
MAX31865 rtd(PT100_CS_PIN); // PT100-термистор на аппаратном SPI
...
void setup() {
  ...
  rtd.begin(); // Запускаем объект неблокирующей библиотеки PT100-термистора
}
