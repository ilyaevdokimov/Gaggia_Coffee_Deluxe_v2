...
#include <Ticker.h> // Таймер для запуска процедур по интервалу
...
volatile uint64_t deugChangesMillis; // Отметка millis изменения
//volatile uint64_t debugChangesMicros; // Отметка micros изменения
//volatile uint64_t debugValueBegin; // Отметка времени начала отладки значения
...
void setup() {
  ...
  controlPanelUpdater.attach_ms(PAGE_REFRESH_INTERVAL, updateContolPanel); // Таймер процедуры актуализации панели управления (странички) Срабатывает каждые PAGE_REFRESH_INTERVAL мс
}
